...
let consume_all_events buf =
  let r =
    ...
    iter ()
  in
  match r with Ok () -> true | Error (`Msg m) -> false

let yaml =
  map [ bytes ] (fun s ->
      match Yaml.of_string s with Ok y -> (s, y) | Error _ -> bad_test ())
...
let to_from orig =
  ...
  match r with Ok s -> s | Error (`Msg m) -> raise (Failure m)
...
let () =
  ...
  (add_test ~name:"events" [ bytes ] @@ fun buf -> check (events_test buf));
  ()
