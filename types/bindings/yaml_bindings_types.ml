...
struct
  ...
  let scalar_style : Scalar_style.t F.typ =
  ...
  let sequence_style : Sequence_style.t F.typ =
    enum "SEQUENCE_STYLE" "yaml_sequence_style_t" [
      `Any,"ANY"; `Block,"BLOCK"; `Flow,"FLOW" ]

  let mapping_style : Mapping_style.t F.typ =
    enum "MAPPING_STYLE" "yaml_mapping_style_t" [
      `Any,"ANY"; `Block,"BLOCK"; `Flow,"FLOW" ]

  let token_type : Token_type.t F.typ = enum "TOKEN" "yaml_token_type_t" [
